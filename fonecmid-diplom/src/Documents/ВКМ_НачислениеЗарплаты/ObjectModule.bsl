...
Процедура РассчитатьУдержание()
	...
	Запрос.Текст = 
		...
		|	ВКМ_ОсновныеНачисленияДанныеГрафика.Зарплата КАК Зарплата
		|ИЗ
		...
		|		ПО ВКМ_ОсновныеНачисленияДанныеГрафика.НомерСтроки = ВКМ_Удержания.НомерСтроки
		|ГДЕ
		|	ВКМ_ОсновныеНачисленияДанныеГрафика.Регистратор = &Регистратор
		|
		|СГРУППИРОВАТЬ ПО
		|	ВКМ_Удержания.НомерСтроки,
		|	ВКМ_ОсновныеНачисленияДанныеГрафика.Зарплата";
	...
	Пока Выборка.Следующий() Цикл
		...
		ДвижениеУдержание.НДФЛ = Выборка.Зарплата * 13 / 100; 
